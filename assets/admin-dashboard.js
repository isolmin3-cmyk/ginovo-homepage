(function(){
  ...
  function buildShell(){
    ...
    var shell=document.createElement('div');shell.className='admin-shell';shell.innerHTML='<aside class="admin-sidebar"><div class="admin-brand"><span class="admin-brand-badge">G</span> GINOVO ADMIN</div><nav class="admin-nav" aria-label="'+t.sections+'">'+nav+'</nav><div class="admin-sidebar-footer"><button type="button" data-admin-logout>'+t.logout+'</button></div></aside><div class="admin-workspace"><header class="admin-topbar"><div style="display:flex;align-items:center;gap:10px"><button class="admin-mobile-menu" type="button" aria-label="'+t.menu+'" aria-expanded="false">☰</button><h2>'+t.title+'</h2></div><div class="admin-topbar-actions"><span class="admin-mode">'+t.preview+'</span><a href="'+(isEnglish?'./index.html':'./index.html')+'" target="_blank">'+t.site+'</a></div></header><div class="admin-content"><section class="admin-hero"><h1>'+t.title+'</h1><p>'+t.intro+'</p></section></div></div>';
  ...
  document.addEventListener('DOMContentLoaded',function(){buildShell();makeLogin()});
})();
