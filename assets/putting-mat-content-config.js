...
window.getPuttingMatComparisonContent = function () {
  ...
  try {
    ...
    let migrated = false;
    const legacyImages = ['./assets/img-016.png', './assets/img-014.png', './assets/img-015.png'];
    const result = defaults.map((fallback, index) => {
      const current = { ...fallback, ...(saved[index] || {}) };
      if (current.image === legacyImages[index]) {
