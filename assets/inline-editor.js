(function(){
  ...
  var editing=null,originalText=new Map(),pendingMedia=new Map(),originalMedia=new Map(),pendingBackgrounds=new Map(),originalBackgrounds=new Map();
  ...
  function sections(){return Array.from(document.querySelectorAll('main section, body > section, body > footer')).filter(function(s){return !s.closest('header')})}
  ...
  function mediaKey(section,el){if(el.dataset.gtMediaKey)return el.dataset.gtMediaKey;var attr=el.getAttribute('data-media-slot')||el.getAttribute('data-media-image')||el.getAttribute('data-feature90-image')||el.getAttribute('data-card-image')||el.getAttribute('data-practice-image')||el.getAttribute('data-unity-media')||el.id||Array.from(el.classList).filter(function(c){return c!=='gt-edit-media'})[0];return pageKey()+'|'+sectionId(section)+'|'+(el.tagName==='IMG'?'image:':'video:')+(attr||mediaTargets(section).indexOf(el))}
  function backgroundKey(section){return pageKey()+'|'+sectionId(section)+'|background'}
  ...
  async function applySaved(){var data=store();for(var section of sections()){for(var el of textTargets(section)){var value=data[elementKey(section,el)];if(typeof value==='string')setTextLines(el,value)}for(var link of linkTargets(section)){var href=data[linkKey(section,link)];if(typeof href==='string'&&validLink(href))link.href=href}for(var media of mediaTargets(section)){var group=media.closest('[data-inline-media-group]');var key=group?pageKey()+'|'+sectionId(section)+'|group:'+group.dataset.inlineMediaGroup:mediaKey(section,media);media.dataset.gtMediaKey=key;var record=data[key],blob=recordBlob(record);if(blob){replaceMediaElement(media,blob,recordKind(record,blob),key);if(group&&record.composite)group.classList.add('gt-composite-replaced')}}if(hasEditableBackground(section)){var background=recordBlob(data[backgroundKey(section)]);if(background) section.style.backgroundImage='url("'+(typeof background==='string'?background:URL.createObjectURL(background))+'")'}}}
  ...
  async function endEdit(save){
    ...
    if(save){
      ...
      try{
        for(var [mediaKeyValue,mediaRecord] of pendingMedia){var mediaUrl=await uploadMedia(mediaRecord.blob);changes[mediaKeyValue]={url:mediaUrl,kind:mediaRecord.kind,composite:!!mediaRecord.composite};if(mirror)changes[mirroredKey(mediaKeyValue)]=changes[mediaKeyValue]}
  ...
  function startEdit(section){if(editing===section)return;if(editing)endEdit(false);editing=section;section.classList.add('gt-edit-active');textTargets(section).forEach(function(el){originalText.set(el,el.innerText);el.classList.add('gt-editable-text');el.setAttribute('contenteditable','true');el.setAttribute('spellcheck','true')});mediaTargets(section).forEach(function(media){addMediaEditor(section,media)});addBackgroundEditor(section);addLinkEditor(section);document.querySelector('.gt-edit-toolbar').classList.add('is-editing');section.scrollIntoView({behavior:'smooth',block:'start'})}
  ...
  async function publishChanges(changes){
    ...
    for(var offset=0;offset<rows.length;offset+=100){
      var result=await client.from('site_content').upsert(rows.slice(offset,offset+100),{onConflict:'content_key'});
      if(result.error)throw result.error;
    }
  ...
  document.addEventListener('DOMContentLoaded',async function(){
    var loaded=false;
    try{await loadPublishedContent();loaded=true;await applySaved()}catch(error){console.error('GINOVO content load failed',error)}
